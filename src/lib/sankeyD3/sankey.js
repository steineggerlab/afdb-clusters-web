...
import {min, max, sum, group, ascending, descending} from "d3-array";
...
export function sankey() {
  ...
  function computeNodeLinks() {
    ...
    links.forEach(function(link) {
      ...
      link.inactive = false;
      if (typeof source === "number") source = link.source = nodes.find(n => n.id === source);
      if (typeof target === "number") target = link.target = nodes.find(n => n.id === target);
  ...
  function computeNodeDepths(iterations) {
    ...
    if (orderByPath) {
      ...
      for (let i = 0; i < nodesByBreadth.length; ++i) {
          nodesByBreadth[i] = [];
      }

      // Add 'invisible' nodes to account for different depths
      for (let posX = 0; posX < max_posX; ++posX) {
          for (let j = 0; j < nodes.length; ++j) {
              ...
              let no_intermediary_nodes = node.sourceLinks.length && node.sourceLinks[0].target.posX > node.posX +1;
              let no_end_nodes = !node.sourceLinks.length && node.posX < max_posX;

              if (no_intermediary_nodes || no_end_nodes) {
                let end_node = no_intermediary_nodes? node.sourceLinks[0].target.posX : max_posX;
                  for (let new_node_posX = node.posX+1; new_node_posX < end_node; ++new_node_posX) {
      ...
      nodesByBreadth = 
        Array.from(group(nodes, d => d.x), ([key, values]) => values)
          .sort(function(a, b) { return descending(a[0].x, b[0].x); });
